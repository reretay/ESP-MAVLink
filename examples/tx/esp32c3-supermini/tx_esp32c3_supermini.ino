...
static const int MAV_TX_PIN = 21; // Pixhawk RX -> ESP TX
    ...
    if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status)) {
      switch (msg.msgid) {
        case MAVLINK_MSG_ID_GPS_RAW_INT: {
          ...
          pkt.lat = gps.lat / 1e7f;
          pkt.lon = gps.lon / 1e7f;
          pkt.alt = gps.alt / 1000.0f;
          break;
        }
...
void setup() {
  ...
  pkt = {0,0,0,0};
  Serial.println("TX(ESP32-C3 SuperMini) ready");
}
